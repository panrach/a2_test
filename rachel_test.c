...
/*
4.1: root node becomes max
4.2: biggest index becomes min
*/
...
void test_changePriority() {
    ...
    // 4.1
    changePriority(heap, 1, 10);
    int expected[9] = {2, 3, 4, 6, 7, 8, 5, 9, 10}; 
    printf("4.1:%s\n", compare(heap, expected, heap->size) ? " PASSED": " FAILED");


    // 4.2
    changePriority(heap, 9, 0);
    int expected2[9] = {0, 2, 4, 3, 7, 8, 5, 9, 6}; 
    printf("4.2:%s\n", compare(heap, expected2, heap->size) ? " PASSED": " FAILED");
